...
for filename in os.listdir(path):
    ...
    #Plot angular velocity with respect to time. 
    ax1.plot(state_history['t'], state_history['motorTorque1'])
    ax1.set_xlabel('time[sec]')
    ax1.set_ylabel('Motor torque [N/m]')
    ax1.ticklabel_format(style='sci', axis='both', scilimits=(0,0))
    ax1.grid()

    ax2.plot(state_history['t'], state_history['motorTorque2'])
    ax2.set_xlabel('time[sec]')
    ax2.set_ylabel('Motor torque [N/m]')
    ax2.ticklabel_format(style='sci', axis='both', scilimits=(0,0))
    ax2.grid()

    ax3.plot(state_history['t'], state_history['motorTorque3'])
    ax3.set_xlabel('time[sec]')
    ax3.set_ylabel('Motor torque [N/m]')
    ax3.ticklabel_format(style='sci', axis='both', scilimits=(0,0))
    ax3.grid()
    
    ax4.plot(state_history['t'], state_history['motorTorque4'])
    ax4.set_xlabel('time[sec]')
    ax4.set_ylabel('Motor torque [N/m]')
    ax4.ticklabel_format(style='sci', axis='both', scilimits=(0,0))
    ax4.grid()

    # ax5.plot(state_history['t'], state_history['motorTorque5'])
    # ax5.set_xlabel('time[sec]')
    # ax5.set_ylabel('Motor torque [N/m]')
    # ax5.ticklabel_format(style='sci', axis='both', scilimits=(0,0))
    # ax5.grid()

    # ax6.plot(state_history['t'], state_history['motorTorque6'])
    # ax6.set_xlabel('time[sec]')
    # ax6.set_ylabel('Motor torque [N/m]')
