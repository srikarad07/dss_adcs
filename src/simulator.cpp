...
simulatorInput checkSimulatorInput( const rapidjson::Document& config )
{
    ...
    const Real principleInertia
        = find( config, "principle_inertia" )->value.GetDouble( );
    std::cout << "Principle Inertia            " << principleInertia
              << " [kg m^2]" << std::endl;
    // Extract the initial state vector. 
    const Real initialStateVector
        = find( config, "initial_state_vector")->value.GetDouble(); 
    std::cout << "Initial State Vector           " << initialStateVector
              << "[rad] & [rad/sec]" << std::endl;
